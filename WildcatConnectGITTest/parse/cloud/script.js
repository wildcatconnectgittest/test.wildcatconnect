		...
		create: function(firstName, lastName, email, username, password) {
			if (! firstName || ! lastName || ! username || ! password || ! email) {
				...
				query.find().then(function(usersA) {
					...
					window.userCount += usersD.length;
					if (window.userCount > 0) {
						alert("This username or email has already been used. Please try again.");
						$("#signupButton").html("Register");
						$("#spinnerDiv").html("");
						window.userCount = 0;
					} else {
				...
				for (var i = 0; i < structures.length; i++) {
					...
					button.onclick = (function() {
					    ...
					    return function(e) {
							...
							var decrypted = CryptoJS.AES.decrypt(password, "Secret").toString(CryptoJS.enc.Utf8);

							// Decrypted has the correct password...

							Parse.Cloud.run('registerUser', { "username" : user.get("username") , "password" : decrypted , "email" : user.get("email") , "firstName" : user.get("firstName") , "lastName" : user.get("lastName") }, {
							  success: function() {
							    alert("User approved!");
							    $("#spinnerDiv").html("");
